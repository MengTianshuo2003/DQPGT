...
def my_summary(test_model, H = 256, W = 256, C = 3, N = 1):
    ...
    inputs = torch.rand((N, C, H, W), device='cuda')
    flops = FlopCountAnalysis(model,inputs)
    n_param = sum([p.nelement() for p in model.parameters()])
    total_flops = flops.total()
    unsupported = dict(flops.unsupported_ops())
    print(f'GMac:{total_flops/(1024*1024*1024)}')
    print(f'Params:{n_param}')
    print(f'Unsupported operators:{unsupported}')
    if any('convolution' in name or 'conv2d' in name for name in unsupported):
        raise RuntimeError(
            'Profiler did not count every convolution; B-table FLOPs are invalid.')
