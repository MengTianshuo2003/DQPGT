...
def read_img2(env, path, size=None):
    """Read a NumPy image or an image from LMDB.

    NumPy image files are expected to contain a numeric HW or HWC array.  SMID,
    SID, and SDSD do not use object arrays, so pickle loading is deliberately
    disabled.  Apart from being safer, this also lets us turn a cryptic pickle
    error into an actionable dataset-corruption error.
    ...
    if env is None:  # img
        try:
            ...
            raise RuntimeError(
                'Failed to load NumPy image {!r} ({} bytes). The file is '
                'empty, truncated, or is not a valid .npy file. Run '
                '`python scripts/validate_smid_npy.py --lq-root <SMID_LQ_np> '
                '--gt-root <SMID_Long_np>` and replace every reported file '
                'from a complete dataset copy.'.format(path, file_size)) from exc

        if not isinstance(img, np.ndarray):
            raise TypeError(
                'Expected a NumPy array in {!r}, but found {!r}.'.format(
                    path, type(img).__name__))
        if img.ndim not in (2, 3) or 0 in img.shape:
            raise ValueError(
                'Expected a non-empty HW or HWC image array in {!r}, but got '
                'shape {}.'.format(path, img.shape))
        if size is not None:
            img = cv2.resize(img, (size[0], size[1]))
