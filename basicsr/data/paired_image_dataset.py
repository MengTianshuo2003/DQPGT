...
from basicsr.data.ablation_perturbations import (
    D2_PERTURBATION_KINDS, apply_ablation_perturbation)
...
class Dataset_PairedImage(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.d2_perturbation = None
        perturbation_opt = opt.get('d2_perturbation')
        if perturbation_opt is not None:
            if not isinstance(perturbation_opt, dict):
                raise TypeError('d2_perturbation must be a mapping.')
            unknown_keys = set(perturbation_opt) - {'enabled', 'kind', 'seed'}
            if unknown_keys:
                raise ValueError(
                    'Unsupported d2_perturbation option(s): '
                    f'{sorted(unknown_keys)}')
            if perturbation_opt.get('enabled', True):
                if opt['phase'] == 'train':
                    raise ValueError(
                        'D2 perturbations are evaluation-only and must not '
                        'be enabled for a training dataset.')
                kind = perturbation_opt.get('kind')
                if kind is None:
                    raise ValueError(
                        'd2_perturbation.kind is required when enabled.')
                kind = str(kind)
                if kind not in D2_PERTURBATION_KINDS:
                    raise ValueError(
                        f'Unsupported d2_perturbation.kind: {kind}. '
                        f'Supported values: {D2_PERTURBATION_KINDS}')
                self.d2_perturbation = {
                    'kind': kind,
                    'seed': int(perturbation_opt.get('seed', 100))
                }
        ...
        # Optional reproducible split for legacy/other experiments. The current
        # LOLv2-syn protocol does not set these options: it trains on all Train
        # pairs and validates directly on the official Test set.
        split_subset = self.opt.get('split_subset')
        if split_subset is not None:
            if split_subset not in ('train', 'val'):
                raise ValueError('split_subset must be either "train" or "val".')
            split_seed = int(self.opt.get('split_seed', 100))
            val_ratio = float(self.opt.get('val_ratio', 0.1))
            if not 0 < val_ratio < 1:
                raise ValueError('val_ratio must be in (0, 1).')
            ordered_paths = sorted(self.paths, key=lambda item: item['gt_path'])
            indices = list(range(len(ordered_paths)))
            random.Random(split_seed).shuffle(indices)
            val_count = max(1, int(round(len(indices) * val_ratio)))
            val_indices = set(indices[:val_count])
            train_paths = [p for i, p in enumerate(ordered_paths) if i not in val_indices]
            val_paths = [p for i, p in enumerate(ordered_paths) if i in val_indices]
            self.paths = train_paths if split_subset == 'train' else val_paths

            manifest_path = self.opt.get('split_manifest')
            if manifest_path:
                manifest = {
                    'seed': split_seed,
                    'val_ratio': val_ratio,
                    'total': len(ordered_paths),
                    'train': [os.path.basename(p['gt_path']) for p in train_paths],
                    'val': [os.path.basename(p['gt_path']) for p in val_paths]
                }
                ...
                if os.path.exists(manifest_path):
                    ...
                    if existing != manifest:
                        raise RuntimeError(
                            f'Existing split manifest does not match the requested split: {manifest_path}')
                else:
    ...
    def __getitem__(self, index):
        ...
        img_gt, img_lq = img2tensor([img_gt, img_lq],
                                    bgr2rgb=True,
                                    float32=True)
        # D2 only perturbs the RGB LQ input. GT remains the original paired
        # normal image. Adding the sample index gives each image a stable
        # realization while preserving identical noise across model variants.
        if self.d2_perturbation is not None:
            img_lq = apply_ablation_perturbation(
                img_lq.unsqueeze(0),
                kind=self.d2_perturbation['kind'],
                seed=self.d2_perturbation['seed'] + index
            ).squeeze(0)
        # normalize
